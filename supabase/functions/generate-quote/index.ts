...
Svara med **1**, **2** eller **3** (eller "granska", "generera", "mer info")`;
    ...
    // ============================================================================
    // FAS 6B: USE PIPELINE ORCHESTRATOR RESULT DIRECTLY
    // ============================================================================
    
    console.log('✅ FAS 6B: Using Pipeline Orchestrator result - all calculations done');
    
    // Use the quote from Pipeline Orchestrator (already mathematically perfect)
    quote = pipelineResult.quote;
    
    // Store jobDef for later use
    const jobDef = pipelineResult.jobDefinition;
    
    // Store allText for later validation
    const allText = (completeDescription + ' ' + actualConversationHistory.map((m: any) => m.content).join(' ')).toLowerCase();
